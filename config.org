...
* Navigation
** Basic Settings
   Quickly move the cursor to the first instance of a character with
   iy-go-to-char:
#+begin_src emacs-lisp
...
*** Appearance
#+begin_src emacs-lisp :exports none something :results output org
...
#+end_src
**** Commands
Need ob-misc for some of this stuff. It's on my github.
#+begin_src emacs-lisp
(require 'ob-misc) 

(global-set-key (kbd "C-c C-v ;") 'org-edit-src-headers)
(global-set-key (kbd "C-c C-v :") 'org-show-headers)
#+end_src
...
#+begin_src emacs-lisp
    ...
    (defconst org-note-template "* %^{Todo} :NOTE:\n%U\n%a\n")
    (defconst org-question-template "* %^{Todo} :NOTE:\n%U\n%a\n")
    (defconst org-task-template "* TODO %^{Todo}\n%U\n%a\n")
    (defconst org-thought-template "* %^{Todo}\n%U\n")
    ...
    (setq org-capture-templates
          `(("g" "General todos and refiles")
            ...
            ("gq" "Quick thought" entry (file nil)
             ,org-thought-template
             :immediate-finish t
             )))
#+end_src
#+begin_src emacs-lisp
