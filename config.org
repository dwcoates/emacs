...
2. =C-h c=    ~describe-command~          (replaces ~describe-key-briefly~)
...
5. =C-h M-k=  ~describe-keymap~
   ...
   major/minor mode.
7. =C-h p=    ~describe-personal-keybindings~
   Describe custom bindings created.
6. =C-h M-l=  ~find-function-on-key~
...
7. =C-h o=    ~describe-option~
   Describe user variables.
8. =C-h C-o=  ~describe-option-of-type~
   Describe user variables, but first narrow them by type.
9. =C-h M-c=  ~describe-copying~          (replaces =C-h C-c=)
   Create a buffer that contains the entire GNU Public License.
10. =C-h C-c=  ~describe-key-briefly~      (replaces =C-h c=)
