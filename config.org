...
* SSH
** Tramp
I edit ~/.ssh/config to add the following
Host repo
...
#+END_SRC
* On Startup
This launches an emacs server. Run an emacs daemon on startup (emacs --daemon), then run emacsclient to spawn emacs sessions (nearly) instantly. There is a single overhead cost of typical startup timen (~2 seconds) on login, but it means means I can now use emacs for quick edits. I bind `emacsclient -c -a emacs' to a keyboard shortcut.  
#+begin_src emacs-lisp
(server-start)
#+end_src
#+begin_src emacs-lisp
  ...
  (defun dwc-create-frame (&optional filename)
  ...
  (defun  dwc-org-frame (org-filename)
