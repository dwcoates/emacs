...
#+begin_src emacs-lisp
  (defun org-notes-sp-wrap-with-pair (pair key &optional beg end)
    (if (or (use-region-p) (and beg end (> end beg)))
        (let* ((arg (or current-prefix-arg 1))
               (active-pair (progn (--first (equal (car it) pair)
                                            sp-pair-list)))
               (rb (or beg (region-beginning)))
               (re (or end (region-end))))
          (goto-char re)
          (save-excursion
...
#+begin_src emacs-lisp
  (defun org-notes-wrap-with-meta-latex-and-execute (beg end &optional key long-latex)
    (let ((open-delim (if long-latex "\\[" "$")))
      (when (org-notes-sp-wrap-with-pair open-delim key beg end)
    ...
    `(progn
       ...
       (define-key org-mode-map (kbd ,key)
         (lambda (beg end)
           ...
           (print beg) 
           (print end)
           (org-notes-sp-wrap-with-pair ,open ,key beg end)))
         ...
         (lambda (beg end)
           (interactive (if (region-active-p)
                            (list (region-beginning)
                                  (region-end))
                          (list nil nil)))
           (let*  ((latex (org-notes-sp-wrap-with-pair
                          ,open ,(upcase key) beg end))
                   (lb (plist-get latex :beg))
...
#+begin_src emacs-lisp
  ;; (defun insert-latex-long (beg end)
  ;;   (interactive "r")
  ;;   (if (use-region-p)
  ;;       (progn
  ;;         (insert-char-with-wrap nil "\\[" "\\]" beg end)
  ;;         (org-toggle-latex-fragment))
  ;;     (self-insert-command 1)))

  ;; (defun insert-latex-short (beg end)
  ;;   (interactive "r")
  ;;   (if (use-region-p)
  ;;       (progn 
  ;;         (insert-char-with-wrap nil "$" "$" beg end)
...
#+begin_src emacs-lisp
  (setq org-todo-keywords
        (quote ((sequence "TODO(t)" "NEXT(n)" "|" "DONE(d)")
                (sequence "LEARN(l)" "REVIEW(r)" "IDEA(e)" "QUIZ(z)" "|" "NOTE(o)" "CLARIFY(c@/@)" )
