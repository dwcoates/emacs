...
#+BEGIN_SRC emacs-lisp
  (eval-after-load 'helm-moccur-mode
    (defun helm-symbol-occur ()
      (interactive)
  ...
  (bind-key "C-c s" 'helm-symbol-occur)
#+END_SRC
...
*** ~shackle~ :popups:
 ...
 #+begin_src emacs-lisp
   (use-package shackle
     ...
     (shackle-mode 1)
     (setq 
      shackle-default-rule '(:inhibit-window-quit t :other t :align right :size 80)
      shackle-rules
      `(;; Util
        ("^\\*.+-Profiler-Report .+\\*$"
   ...
   (use-package smartparens
     ...
     (defun sp-lisp-pair-pred (_ _ _)
       ...
       (not (or (eq major-mode 'emacs-lisp-mode) (eq major-mode 'org-mode)
                (eq major-mode 'magit-mode) (eq major-mode 'c++-mode) (eq major-mode 'c-mode)
...
#+begin_src emacs-lisp
  (add-to-list 'load-path "~/.emacs.d/.local/site-packages/ess/")
  (when (load "ess-site" )
...
#+begin_src emacs-lisp
  (defun current-fen-to-clipboard ()
    (interactive)
    ;(kill-new (chess-pos-to-fen (chess-game-pos chess-module-game (chess-game-index chess-module-game)) ))
    (message "%s" (chess-ply-pos chess-module-game))
    (message "%s" (chess-game-ply chess-module-game))
    )

  (define-key chess-display-mode-map (kbd "C-w") 'current-fen-to-clipboard)
#+end_src
