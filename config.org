...
#+begin_src emacs-lisp
  (defun python-shell-send-paragraph-and-step ()
    ...
    (save-excursion
      (forward-paragraph)
      (let* ((end (point))
             (beg (progn (backward-paragraph) (forward-line) (point)))
  ...
  (defun python-shell-send-code ()
    ...
    (if (and transient-mark-mode 
      ...
      (if (save-excursion (forward-paragraph)
                          (backward-paragraph)
                          (forward-line)
                          (python-info-current-defun))
          (progn 
            (python-shell-send-defun)
            (message (propertize "Sent '%s' function to python interpreter")
                     (propertize (python-info-current-defun) 'face
                                 '(:foreground "#66D9EF"))
                     ))
...
#+RESULTS:
: python-shell-send-code

Replace the normal binding (which is set to ~python-shell-send-file~) with this
improved function.

#+begin_src emacs-lisp
  (define-key python-mode-map (kbd "C-c C-c") 'python-shell-send-code)
#+end_src
