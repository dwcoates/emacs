...
python-mode
#+begin_src emacs-lisp
(setenv "PYTHONPATH" "/usr/bin/python:$HOME/.local/bin/")
#+end_src
elpy
#+BEGIN_SRC emacs-lisp
...
*** Capture
#+begin_src emacs-lisp
  (defvar basic-task-template "* TODO %^{Task:}
  DEADLINE: %^t
  ...
  (defvar basic-nugget-template "* %^{Task:}
  SCHEDULED: %T
  ...
  (defvar basic-task-template-with-source (concat basic-task-template "\nfrom %h")
    "Org capture template for general tasks. Includes a link to file from which capture was initiated."
    )
      ...
      (setq org-capture-templates
            `(("e"
              ...
              ("T"
               "A short-term task to include link to file where captured"
               (id "8a63b75e-fd83-4aa9-8812-0c886301148f")
               ,basic-task-template-with-source
               )
...
#+begin_src emacs-lisp
  ...
  (defun dwc-create-frame (filename)
  ...
  (defun org-school-frame--bare ()
    ...
    (defun leven-frames ()
      ;; dev frame
      (find-file "~/workspace/leven-squash/levenshtein/")
      (split-window-horizontally)
      (windmove-right)
      (find-file "~/workspace/leven-squash/tests/")
      (split-window-below)
      (windmove-down)
      (eshell)
      (shrink-window 10)
      ...
      ;; org frame
      (dwc-create-org-frame "~/workspace/leven-squash/project.org")
      )

  (defun on-emacs-startup ()
    "Default frame scheme on startup."
    (interactive)
    ;; FRAME 1
    ;;(org-frame)
    (leven-frames)
