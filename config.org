...
#+begin_src emacs-lisp
  ...
  (defun isearcher ()
    (interactive)
    (when isearcher-register
      (goto-char isearcher-register)))

  (defun dwc-exchange-point-and-mark ()
    (interactive)
    (if (use-region-p)
...
**** ~smartparens~
 ...
 #+BEGIN_SRC emacs-lisp
   (use-package smartparens
     ...
     (sp-pair "'" nil :when '(eq major-mode (or 'org-mode 'lisp-mode)) :actions nil) 
     (sp-pair "`" nil :when '(eq major-mode (or 'org-mode 'lisp-mode)) :actions nil) 
...
#+begin_src emacs-lisp
  (use-package ein
    ...
    (define-key ein:notebook-mode-map (kbd "C-c v n") 'ein:worksheet-insert-cell-below)
    (define-key ein:notebook-mode-map (kbd "C-c k") 'ein:worksheet-kill-cell)
    (define-key ein:notebook-mode-map (kbd "C-c v p") 'ein:worksheet-insert-cell-above)
    (define-key ein:notebook-mode-map (kbd "C-c v d") 'ein:worksheet-split-cell-at-point)
    (defun switch-color ()
      (interactive)
