...
;; I have this set to be very short, short enough that to trigger the command, the keys have to be pressed 
;; as one motion, and not in sequence. For example, with this setting, I can't (realistically) trigger a      ;; command assigned to "we" by pressing 'w' and 'e' in sequence as I would typing the word naturally.
;; This makes two-handed keys chords a slightly unreliable, at least without practice.
...
;; the original configuration is too close on the keyboard to other-frame
(global-unset-key (kbd "C-x 5 0"))
(global-set-key (kbd "C-x 5 DEL") 'delete-frame)

;; likewise, too close to other-window
