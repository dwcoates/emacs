...
*** Key Chords
$key-chord-one-key-delay$ defines the max time delay between two key presses to be considered a key chord. 
I have this set to be very short, short enough that to trigger the command, the keys pretty much have to 
be pressed as one motion, and not in sequence. For example, with this setting, I can't (realistically) trigger 
a command assigned to "we" key chord by pressing 'w' and 'e' in sequence as I would typing them in a word 
naturally. This makes two-handed keys chords slightly unreliable, at least without practice. So generally, my
key chords consist of keys adjacent to one another.
#+BEGIN_SRC emacs-lisp
(use-package key-chord
  :ensure t
  :init
  (progn
    (setq key-chord-one-key-delay 0.035)
    (key-chord-mode 1))
)
#+END_SRC
** Helm
...
#+BEGIN_SRC emacs-lisp
(use-package helm
  :commands (helm-grep helm-eshell)
  :bind (("M-x" . helm-M-x)
...
(global-set-key (kbd "C-c i w") 'helm-wikipedia-suggest)
(global-set-key (kbd "C-c i g") 'helm-google-suggest)
(global-set-key (kbd "C-c i s") 'helm-surfraw)
...
(use-package windmove
  :commands
  ;; This is here because alternative commands (key chords) do not trigger package autoload.
  (windmove-left windmove-right windmove-up windmove-down)
  :init
  (bind-key "C-x w j" 'windmove-left)
  (bind-key "C-x w l" 'windmove-right)
  (bind-key "C-x w i" 'windmove-up)
  (bind-key "C-x w k" 'windmove-down)
...
(use-package golden-ratio
  ...
  :init
  (bind-key "C-x w g SPC" 'golden-ratio)    ;; Resize buffers according to golden-ratio
  (bind-key "C-x w g m" 'golden-ratio-mode) ;; Enable/disable golden-ratio-mode
  :config
  (progn
...
(use-package zygospore
  :bind ("C-x 1" . zygospore-toggle-delete-other-window)
  )
#+END_SRC
** Functions
