...
;; I have this set to be very short, short enough that to trigger the command, the keys 
;; have to be pressed as one motion, and not in sequence. For example, with this setting, 
;; I can't (realistically) trigger a      ;; command assigned to "we" by pressing 'w' and 'e' 
;; in sequence as I would typing the word naturally. This makes two-handed keys chords a 
;; slightly unreliable, at least without practice.
