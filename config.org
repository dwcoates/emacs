#+Title: This is my emacs configuration file.
...
Great, simple package. Makes =C-w= and =M-w= act as ~kill-whole-line~ or copy whole
line when no region is active, and normal ~kill-region~ or ~yank~ otherwise.
...
#+begin_src emacs-lisp
  (defun insert-char-with-wrap (char &optional wrap-char-beg wrap-char-end)
    (let ((wrap-char-beg (or wrap-char-beg char))
          (wrap-char-end (or wrap-char-end char))
          (beg-marker (set-marker (make-marker) (region-beginning)))
          (end-marker (set-marker (make-marker) (region-end))))
  ...
  (defun insert-dollar ()
    (interactive)
    (insert-char-with-wrap (string-to-char "$")))
  ...
  (defun insert-super-script ()
    (interactive)
    (let ((end 
           (if (region-active-p) (set-marker (make-marker) (region-end)) 
             nil)))
      (insert-char-with-wrap "^" "^{" "} ")
      (when end
        (goto-char (marker-position end))
        (forward-char)))) 

  (defun insert-sub-script ()
    (interactive)
    (let ((end 
           (if (region-active-p) (set-marker (make-marker) (region-end)) 
             nil)))
      (insert-char-with-wrap "_" "_{" "} ")  
      (when end
        (goto-char (marker-position end))
        (forward-char))))

  (define-key org-mode-map (kbd "\\") 'insert-latex)
  (define-key org-mode-map (kbd "^") 'insert-super-script)
  (define-key org-mode-map (kbd "_") 'insert-sub-script)
  (define-key org-mode-map (kbd "/") 'insert-italics)
  (define-key org-mode-map (kbd "=") 'insert-verbatim)
  (define-key org-mode-map (kbd "*") 'insert-bold)
  (define-key org-mode-map (kbd "$") 'insert-dollar)
#+end_src
...
#+begin_src emacs-lisp
  (setq org-todo-keywords
        (quote ((sequence "TODO(t)" "NEXT(n)" "|" "DONE(d)")
                (sequence "NOTE(o)" "LEARN(l)" "|" "UNDERSTOOD(d)")
